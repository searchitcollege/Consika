...
if ($is_ajax && $_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($action === 'add_employee') {
        ...
        $stmt->bind_param(
            'sssssssssssdddss',
            $employee_code, $full_name, $id_number, $phone, $alt_phone, $email,
            $position, $department, $specialization,
            $hire_date, $contract_type,
            $hourly_rate, $daily_rate, $monthly_salary,
    ...
    if ($action === 'edit_employee') {
        ...
        $stmt->bind_param(
            ...
            $position, $department, $specialization, $contract_type,
            $hourly_rate, $daily_rate, $monthly_salary,
    ...
    if ($action === 'assign_project') {
        ...
        $stmt = $db->prepare("
            ...
            VALUES (?, ?, ?, ?, ?, 'Active', ?, NOW())
        ");
        $stmt->bind_param('iissdii', $project_id, $employee_id, $role_name, $start_date, $daily_rate, $user_id,
    ...
    // GET EMPLOYEE 
    if ($action === 'get_employee' && $_SERVER['REQUEST_METHOD'] === 'GET' || 
                    ...
                    <div class="row g-3">
                            ...
                            <div class="card h-100">
                                ...
                                <div class="card-body small">
                                    <table class="table table-sm table-borderless mb-0">
                                        <tr><th class="text-muted w-40">Code</th><td id="veCode"></td></tr>
                                        <tr><th class="text-muted">ID Number</th><td id="veIdNum"></td></tr>
                                        <tr><th class="text-muted">Phone</th><td id="vePhone"></td></tr>
                                        <tr><th class="text-muted">Alt Phone</th><td id="veAltPhone"></td></tr>
                                        <tr><th class="text-muted">Email</th><td id="veEmail"></td></tr>
                                        <tr><th class="text-muted">Emergency</th><td id="veEmergency"></td></tr>
                                    </table>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="col-md-6">
                            <div class="card h-100">
                                ...
                                <div class="card-body small">
                                    <table class="table table-sm table-borderless mb-0">
                                        <tr><th class="text-muted w-40">Department</th><td id="veDept"></td></tr>
                                        <tr><th class="text-muted">Specialization</th><td id="veSpec"></td></tr>
                                        <tr><th class="text-muted">Contract</th><td id="veContract"></td></tr>
                                        <tr><th class="text-muted">Hire Date</th><td id="veHireDate"></td></tr>
                                        <tr><th class="text-muted">Hourly Rate</th><td id="veHourly"></td></tr>
                                        <tr><th class="text-muted">Daily Rate</th><td id="veDaily"></td></tr>
                                        <tr><th class="text-muted">Monthly</th><td id="veMonthly"></td></tr>
                                    </table>
                                </div>
                            </div>
                        </div>
...
function viewEmployee(id) {
    ...
    $.get(PAGE_URL, { action: 'get_employee', employee_id: id })
        .done(function (res) {
            if (!res.success) { alert(res.error || 'Failed to load employee.'); return; }
            var e = res.employee;
            ...
            var sc = { Active: 'bg-success', Inactive: 'bg-secondary', 'On Leave': 'bg-warning' };
...
$('#editEmpBtn').on('click', function () {
    var $btn = $(this), $spinner = $('#editEmpSpinner'), $alert = $('#editEmpAlert');
    ...
    .done(function (res) {
        if (res.success) {
            ...
            setTimeout(function () { $('#editEmployeeModal').modal('hide'); location.reload(); }, 1000);
        } else {
    ...
    .always(function () { $btn.prop('disabled', false); $spinner.addClass('d-none'); });
});
...
$('#assignBtn').on('click', function () {
    var $btn = $(this), $spinner = $('#assignSpinner'), $alert = $('#assignAlert');
    ...
    .done(function (res) {
        if (res.success) {
            ...
            setTimeout(function () { $('#assignProjectModal').modal('hide'); }, 1200);
        } else {
...
$('#addEmpBtn').on('click', function () {
    var $btn = $(this), $spinner = $('#addEmpSpinner'), $alert = $('#addEmpAlert');
    ...
    .done(function (res) {
        if (res.success) {
            ...
            setTimeout(function () { $('#addEmployeeModal').modal('hide'); location.reload(); }, 1200);
        } else {
