    ...
    if (isset($_POST['action'])) {
        switch ($_POST['action']) {
            case 'add':
                ...
                $company_type = $db->escapeString($_POST['company_type']);
                $registration_number = $db->escapeString($_POST['registration_number']);
                ...
                $sql = "INSERT INTO companies (company_name, company_type, registration_number, tax_number, address, phone, email, website, established_date, status) 
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                $stmt = $db->prepare($sql);
                $stmt->bind_param("ssssssssss", $company_name, $company_type, $registration_number, $tax_number, $address, $phone, $email, $website, $established_date, $status);
                ...
                if ($stmt->execute()) {
                    ...
                    $user_sql = "INSERT INTO users (username, password, email, full_name, role, company_id, status) 
                                VALUES (?, ?, ?, ?, 'CompanyAdmin', ?, 'Active')";
                    $user_stmt = $db->prepare($user_sql);
                    $full_name = $company_name . ' Administrator';
                    $user_stmt->bind_param("ssssi", $default_username, $default_password, $default_email, $full_name, $company_id);
                    $user_stmt->execute();
                    
                    $_SESSION['success'] = 'Company added successfully with default admin user';
            ...
            case 'edit':
                ...
                $sql = "UPDATE companies SET company_name = ?, company_type = ?, registration_number = ?, tax_number = ?, 
                        address = ?, phone = ?, email = ?, website = ?, established_date = ?, status = ? 
                        WHERE company_id = ?";
                $stmt = $db->prepare($sql);
                $stmt->bind_param("ssssssssssi", $company_name, $company_type, $registration_number, $tax_number, $address, $phone, $email, $website, $established_date, $status, $company_id);
...
<head>
    ...
    <style>
        ...
        .company-card.estate { border-left-color: #4361ee; }
        .company-card.procurement { border-left-color: #4cc9f0; }
        .company-card.works { border-left-color: #f72585; }
        .company-card.blockfactory { border-left-color: #43e97b; }
        ...
        .company-icon.estate { background: linear-gradient(135deg, #4361ee, #3f37c9); }
        .company-icon.procurement { background: linear-gradient(135deg, #4cc9f0, #4895ef); }
        .company-icon.works { background: linear-gradient(135deg, #f72585, #b5179e); }
        .company-icon.blockfactory { background: linear-gradient(135deg, #43e97b, #38f9d7); }
